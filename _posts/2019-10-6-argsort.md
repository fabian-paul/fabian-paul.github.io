...
```
Note that `identity` is an ordered list and contains the same integers as `order` (but in a different order). Now from what we have learned above, we know that the list of indices that sorts the elements of `order` is `np.argsort(order)`. Therefore
...
sorted_data
>>> array([ 0.53,  2.78,  9.51, 57.14, 70.59, 77.09, 79.87, 87.73,
       89.41, 98.36])
```
